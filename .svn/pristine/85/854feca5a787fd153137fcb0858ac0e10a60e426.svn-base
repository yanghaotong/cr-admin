...
public class StockController extends BaseController {
	...
	public ModelAndView toAddStock(HttpSession session) {
		...
		List<PersonInfo> supplierList = supplierService.querySupplierList(personInfo);
		// 返回页面赋值
		// 处理返回值
	...
	public ModelAndView toShowStockIncomePricePage() {
		ModelAndView mv = new ModelAndView("stock/validateIncomePricePassword.ftl");
		return mv;
	}
	...
	// @LogAnnotation(option = "跳转到修改库存页面", optionType = OptionType.pageIn)
	public String toInComeStock(Integer stockId, HttpSession session, Model model) {
		Stock pstock = new Stock();
		pstock.setStockId(stockId);
		pstock.setCompanyId(Util.getCompanyId(session));
		pstock.setIsdelete("0");// 有效的
		...
		model.addAttribute("supplierList", supplierList == null ? "" : supplierList);
	...
	public Object addStockInfo(Stock stock, HttpSession session) {
		...
		if (stockId != null && stockId > 0) {// 修改
			msg = "修改成功！";
			if (StringUtils.isNotBlank(pageType) && StringUtils.equalsIgnoreCase(pageType, "inCome")) {// 进货
			...
			stockIncome.setIncomeStockQuantity(stockInfo.get(0).getStockQuantity());
			stockIncome.setIncomeTime(DateUtils.getCurrDate());
	...
	public ModelAndView toStockStatisticList() {
		ModelAndView mv = new ModelAndView("statistics/stock/stockStatisticsList.ftl");
		return mv;
	}
	...
	@ResponseBody
	// @LogAnnotation(option = "查询库存列表", optionType = OptionType.query)
	public Object outStockList(HttpSession session, String endTime, String startTime, String dateType) {
		...
		for (OutStockVo stockVo : list) {
			itemSum = itemSum.add(stockVo.getItemSum() == null ? new BigDecimal(0) : stockVo.getItemSum());// 已使用总成本
			stockCost = stockCost.add(stockVo.getCost());// 剩余总成本
		}
	...
	// @LogAnnotation(option = "跳转到修改库存页面", optionType = OptionType.pageIn)
	public String toOutStock(Integer stockId, String startTime, String endTime, String dateType, HttpSession session,
			Model model) {
	 ...
	 * 获取销货记录列表
	 * 
	 * @param @param stockId @param @param session @param @param
	 * model @param @return @return Object @author scn @time
	 * 2017-9-7下午3:23:52 @throws
	 */
	@RequestMapping("/toOutStockItemList.do")
	@ResponseBody
	public Object getOutStockList(Integer stockId, String startTime, String endTime, String dateType,
			HttpSession session) {
		...
		pstock.setCompanyId(Util.getCompanyId(session));
		// 根据库存id查询每辆车的使用情况
		List<OutStockVo> queryStock = stockService.queryOutStockItemInfo(pstock);
		Map<String, Object> returnMap = new HashMap<String, Object>();
	 ...
	 * 跳转到库存进货统计页面
	 * 
	 * @param @return @return ModelAndView @author scn @time
	 * 2017-9-9下午11:00:55 @throws
	 */
	...
	@ResponseBody
	// @LogAnnotation(option = "查询库存列表", optionType = OptionType.query)
	public Object getComingStockList(HttpSession session, String endTime, String startTime, String dateType) {
		OutStockVo stock = new OutStockVo();
		stock.setCompanyId(Util.getCompanyId(session));
	...
	// @LogAnnotation(option = "跳转到修改库存页面", optionType = OptionType.pageIn)
	public String toIncomeItemStockList(Integer stockId, String startTime, String endTime, HttpSession session,
			Model model) {
		model.addAttribute("stockId", stockId);
		model.addAttribute("startTime", startTime);
		model.addAttribute("endTime", endTime);
	...
	@ResponseBody
	// @LogAnnotation(option = "查询库存列表", optionType = OptionType.query)
	public Object getIncomeItemStock(HttpSession session, Integer stockId, String endTime, String startTime) {
		OutStockVo stock = new OutStockVo();
		stock.setCompanyId(Util.getCompanyId(session));
		stock.setEndTime(DateUtils.format(endTime));
		stock.setStartTime(DateUtils.format(startTime));
	...
	@ResponseBody
	// @LogAnnotation(option = "跳转到修改库存页面", optionType = OptionType.pageIn)
	public void expStockComeList(HttpSession session, String endTime, String startTime, String dateType,
		...
		try {
			...
			if (StringUtils.isBlank(startTime) && StringUtils.isBlank(endTime)) {
				...
				fileName = startTime == null ? "" : startTime + "-" + endTime == null ? "" : endTime + "库存进货记录";
			}
			ExcelUtil.listToExcel(list, fieldMap, fileName, 65525, response, fileName);
	...
	@ResponseBody
	// @LogAnnotation(option = "跳转到修改库存页面", optionType = OptionType.pageIn)
	public void expStockComeItemList(HttpSession session, Integer stockId, String endTime, String startTime,
		...
		try {
			...
			if (StringUtils.isBlank(startTime) && StringUtils.isBlank(endTime)) {
				...
				fileName = startTime == null ? "" : startTime + "-" + endTime == null ? "" : endTime + stockId + "进货记录";
			}
			ExcelUtil.listToExcel(list, fieldMap, fileName, 65525, response, fileName);
		} catch (ExcelException e) {
			// TODO Auto-generated catch block
	...
	private Integer doSupplier(Stock stock) {
		...
		List<PersonInfo> oldSupplierList = supplierService.querySupplierList(info);
		info.setPerTel(stock.getSupplierTel());

		if (stock.getSupplierId() != null) {
			id = stock.getSupplierId();
		} else if (oldSupplierList != null && oldSupplierList.size() > 0 && oldSupplierList.get(0) != null) {
