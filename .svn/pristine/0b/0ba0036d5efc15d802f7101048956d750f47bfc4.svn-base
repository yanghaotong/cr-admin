...
$("tbody").on(
		...
		function() {
			...
			if (hrefId == "track") {
				layer_show_refresh_click('详细销货记录',
						'/cr-admin/stock/toOutItemStock.do?&stockId=' + stockId
								+ "&startTime=" + startTime + "&endTime="
								+ endTime + "&dateType=" + dateType, '', '',
						stockTable, 'false');
			}
		});
...
function initStatisticsData() {
	...
	$('.table-sort tbody').on(
			...
			function() {
				...
				if (hrefId == "track") {
					layer_show_refresh_click('详细销货记录',
							'/cr-admin/stock/toOutItemStock.do?&stockId='
									+ stockId + "&startTime=" + startTime
									+ "&endTime=" + endTime + "&dateType="
									+ dateType, '', '', stockTable, 'false');
				}
			});
	...
	var startTime = document.getElementById("startTime").value;
	var endTime = document.getElementById("endTime").value;
	var url = "/cr-admin/stock/outStockList.do";
	$.ajax({
		url : url,
		type : "post",
		data : {
		...
		success : function(data) {
			$("#stockCost").val(data.stock.stockCost);
			$("#itemSum").val(data.stock.itemSum);
			$("#profit").val(data.stock.profit);
		}
	});
	var returntable = $("#outStockListTable")
			.DataTable(
					{
						"destroy" : true,
						"ajax" : {
						...
						"columnDefs" : [
								...
								},
								{
									...
									targets : -1,
									render : function(data, type, row, meta) {
										var tdHtml = "<td class=\"td-status\"><a title=\"详细销货记录\" href=\"javascript:;\" class=\"track\"><span class=\"label label-success radius\">详细销货记录</span></a>"
												+ "</td>";
										return tdHtml;
									}
								},
